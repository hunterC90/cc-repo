...
function checkWatched(itemID, watchObj)
  ...
  ae2 = getAe2()
	watchList[itemID].crafting = ae2.getCraftables({["name"]=stockedItem.name, ["label"]=stockedItem.label, ["maxDamage"]=stockedItem.maxDamage})[1].requesting() > 0
...
function updateItem(item)
	...
	if ae2 ~= nil then
		...
		if stockedItems[id].size < item.quantity then
			...
			if (not (recipe.requesting() > 0)) and CPUInfo().available > 0 then
				recipe.request(stockedItems[id].batch)
			end
			if watchList[id] == nil then
				watchList[id] = {
					crafting = recipe.requesting() > 0,
					cycles = 1
				}	
			end
		else
